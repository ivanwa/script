...
# Release:
# 2016-04-06 merge [2015-11-04 security_script_v2.sh]
# 2016-04-07 add [sshd_conf|sysbanner_conf]
# 2016-04-20 fix [RHEL7 system-auth bug fix]
...
check)
	...
	pam3=$?
	#account lock config
	sed -n $(echo $(sed -n '/^auth[[:space:]]*requisite[[:space:]]*pam_succeed_if.so.*/{=;q}' /etc/pam.d/system-auth)+1|bc)p /etc/pam.d/system-auth|egrep "^auth[[:space:]]*required[[:space:]]*pam_tally2.so[[:space:]]*deny=5[[:space:]]*onerr=fail[[:space:]]*unlock_time=180" &>/dev/null
...
check)
	...
	#adm2=$?
	egrep "^\*.\*[[:space:]]*@172.29.27.15:514" /etc/rsyslog.conf &>/dev/null
	log=$?
	role=0
...
set)
	...
	if [ $log -ne 0 ]
	then
		$cmdpre sed -i '$a*.* @172.29.27.15:514' /etc/rsyslog.conf &>/dev/null
	fi
	if [ $role -ne 0 ]
	then
...
check)
	...
	then
		...
		ntp1=$?
		egrep -v '^server[[:space:]]*172.28.25.*[[:space:]]*$' /etc/ntp.conf|grep '^server' &>/dev/null
		ntp2=$?
		egrep '^server[[:space:]]*172.28.25.*[[:space:]]*$' /etc/ntp.conf &>/dev/null
		ntp3=$?
...
set)
	...
	then
		...
		if [ $ntp2 -eq 0 ]
		then
			for match in $(egrep -v '^server[[:space:]]*172.28.25.*[[:space:]]*$' /etc/ntp.conf|grep '^server');
			do
		...
		if [ $ntp3 -ne 0 ]
		then
				$cmdpre sed -i '$aserver 172.28.25.27 iburst' /etc/ntp.conf
        $cmdpre sed -i '$aserver 172.28.25.28 iburst' /etc/ntp.conf
        $cmdpre sed -i '$aserver 172.28.25.29 iburst' /etc/ntp.conf
        $cmdpre sed -i '$aserver 172.28.25.30 iburst' /etc/ntp.conf
		fi
...
set)
	...
	if [ $bug_set -eq 1 ]
	then
		yum update -y  bash openssl openssh glibc  &>/dev/null
		if [ $? -eq 0 ]
		then
...
case $1 in
check)
    $cmdpre grep '^PermitRootLogin[[:space:]]*no$' /etc/ssh/sshd_config &>/dev/null
    if [ $? -eq 0 ]
    then
        color green "The sshd config check:             [pass]"
        backup=0
    else
        color red "The sshd config check:             [fail]"
        backup=1
    fi
    ;;
set)
    sshd_conf check &>/dev/null
    if [ $backup -eq 1 ]
    then
        $cmdpre cp -p /etc/ssh/sshd_config /etc/ssh/sshd_config_bak_$now
        $cmdpre grep '^PermitRootLogin[[:space:]]*' /etc/ssh/sshd_config &>/dev/null
        if [ $? -eq 0 ]
        then
            $cmdpre sed -i 's/^PermitRootLogin.*/PermitRootLogin no/g' /etc/ssh/sshd_config
        else
            $cmdpre sed -i '$aPermitRootLogin no' /etc/ssh/sshd_config
        fi
        $cmdpre service sshd restart &>/dev/null
    fi
    color green "The sshd config set:               [ok]"
	sshd_conf check
    ;;
*) dealerr "sshd_conf" 1
esac
...
check)
    ...
    [ ! -e /etc/issue.net ]
    sysban2=$?
    if [ $sysban1 -eq 0 -a $sysban2 -eq 0 ]
    then
        color green "The sysbanner config check:        [pass]"
        backup=0
    else
        color red "The sysbanner config check:        [fail]"
        backup=1
    fi
    ;;
set)
    sysbanner_conf check &>/dev/null
    if [ $backup -eq 1 ]
    then
        $cmdpre mv /etc/issue /etc/issue_bak_$now
        $cmdpre mv /etc/issue.net /etc/issue.net_bak_$now
    fi
    color green "The sysbanner config set:            [ok]"
	sysbanner_conf check
    ;;
*) dealerr "sysbanner_conf" 1
esac
}
